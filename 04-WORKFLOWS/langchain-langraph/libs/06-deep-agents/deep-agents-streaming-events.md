---
artifact_id: "deep-agents-streaming-events"
artifact_type: "workflow_skill"
version: "1.0.0"
constraints_mapped: ["C1","C5","C7","C8","C9"]
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --file 04-WORKFLOWS/langchain-langraph/libs/deep-agents-streaming-events.md --json"
canonical_path: "04-WORKFLOWS/langchain-langraph/libs/deep-agents-streaming-events.md"
tier: 2
mode_selected: "B1"
prompt_hash: "sha256:deep-agents-streaming-v1.0.0"
...
# 🔄 Deep Agents – Streaming, Eventos e Callbacks

> **Contrato modular**: Artefato filho do Master Agent. Documenta como usar streaming de tokens, eventos de agente, callbacks customizados e integração com LangSmith para visibilidade em tempo real.

---

## 🎯 Propósito
Permitir que agentes MANTIS forneçam feedback em tempo real ao usuário e sejam monitorados via eventos e callbacks.

## 📋 Especificação (SDD)
- **Entradas**: Configuração de streaming, callbacks.
- **Saídas**: Tokens e eventos transmitidos.
- **Side Effects**: Nenhum.
- **Constraints Aplicáveis**: C1 (formato de eventos), C5 (schema), C7 (não quebrar fluxo), C8 (observabilidade), C9 (trace).
- **Dependências**: `langgraph`, `langchain.callbacks`.
...
### 2. Streaming de Eventos com `astream_events`

```python
async for event in agent.astream_events(
    {"messages": [{"role": "user", "content": "Pesquise IA"}]},
    config=config,
    version="v2",
):
    kind = event["event"]
    if kind == "on_chat_model_stream":
        ...
        mantis_log("INFO", "tool_start", event["name"])
    elif kind == "on_tool_end":
        mantis_log("INFO", "tool_end", event["name"])
```
...
class MANTISCallback(BaseCallbackHandler):
    def on_llm_start(self, serialized, prompts, **kwargs):
        mantis_log("INFO", "llm_start", f"Prompts: {len(prompts)}")
    def on_llm_end(self, response, **kwargs):
        mantis_log("INFO", "llm_end", "LLM concluiu")
    def on_llm_error(self, error, **kwargs):
        mantis_log("ERROR", "llm_error", str(error))
    def on_tool_start(self, serialized, input_str, **kwargs):
        mantis_log("INFO", "tool_start", f"Tool: {serialized['name']}")
    def on_tool_end(self, output, **kwargs):
        mantis_log("INFO", "tool_end", f"Output: {output[:100]}...")

agent = create_deep_agent(model="openai:gpt-5.4")
result = agent.invoke(
    {"messages": [{"role": "user", "content": "Teste"}]},
    config={"callbacks": [MANTISCallback()]},
)
```

### 4. Identificação de Agentes em Streaming

```python
# Metadados lc_agent_name disponíveis nos eventos
async for event in agent.astream_events(...):
    agent_name = event.get("metadata", {}).get("lc_agent_name")
    if agent_name:
        print(f"[{agent_name}] ", end="")
```

### 5. Streaming com Subagentes

```python
# Subagentes também emitem eventos com seu próprio lc_agent_name
...
async for event in agent.astream_events(...):
    agent_name = event.get("metadata", {}).get("lc_agent_name", "unknown")
    print(f"[{agent_name}] {event.get('data', {})}")
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --file 04-WORKFLOWS/langchain-langraph/libs/deep-agents-streaming-events.md --json
```
