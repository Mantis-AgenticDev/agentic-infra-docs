...
# VERSIÓN: 1.0.0
# FECHA: 2026-04-14
# AUTOR: MiniMax Agent (Senior Auditor)
#
# ==============================================================================
...
# ------------------------------------------------------------------------------
# SECCIÓN 4: CAPA 2 - FILTRO NORMATIVO C1-C8
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# run_capa2_normative(): Ejecuta toda la Capa 2
# ------------------------------------------------------------------------------
run_capa2_normative() {
    log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_info "CAPA 2: FILTRO NORMATIVO C1-C8"
    log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

    local file="$TARGET_FILE"
    local capa2_failed=false

    # Ejecutar verificadores específicos

    # C3: Zero Hardcode - BLOQUEO CRÍTICO
    if ! check_constraint_c3; then
        log_error "🚫 BLOQUEO CRÍTICO: C3 violado"
        capa2_failed=true
    fi

    # C4: Multi-tenancy - BLOQUEO CRÍTICO
    if ! check_constraint_c4; then
        log_error "🚫 BLOQUEO CRÍTICO: C4 violado"
        capa2_failed=true
    fi

    # C5: Validación declarada
    check_constraint_c5
    ...
    # Invocar validadores externos según tipo
    case "$TARGET_FILE_TYPE" in
        bash)
            run_validator "audit-secrets.sh" "$file"
            ;;
        markdown)
            run_validator "check-wikilinks.sh" "$file"
            ;;
        json)
            run_validator "schema-validator.py" "$file"
            ;;
        yaml)
            run_validator "audit-secrets.sh" "$file"
            ;;
    ...
    # Invocar verify-constraints para verificación general
    if [[ -f "${VALIDATOR_BASE_PATH}/verify-constraints.sh" ]]; then
        run_validator "verify-constraints.sh" "$file"
    fi
    ...
    log_success "✅ CAPA 2 COMPLETADA - Normativa verificada"
    return 0
}
