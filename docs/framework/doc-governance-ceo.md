---
artifact_id: "doc-governance-ceo"
...
canonical_path: "docs/framework/doc-governance-ceo.md"
tier: 1
agent_role: "ceo-orchestrator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/doc-governance-ceo.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 DOC-GOVERNANCE-CEO – Coordinador Maestro de Documentación MANTIS

> **Princípio**: Coordinar central que recibe solicitudes de documentación, las descompone en subtareas, rutea a agentes especializados, fusiona salidas y hace cumplir gates de calidad.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "doc-governance-ceo"
  agent_type: "doc-ceo-orchestrator"
  domain: "docs"
  version: "1.0.0"
  constraints_mapped: ["C1","C2","C3","C4","C5","C6","C7","C8","C9","DOC-C1","DOC-C2","DOC-C3","DOC-C4","DOC-C5","DOC-C6","DOC-C7","DOC-C8","DOC-V1","DOC-V2","DOC-V3"]
  
  skills:
    - id: "docs:orchestration"
      description: "Coordinación central de generación, análisis y mantenimiento documental"
      input_schema: "sdd-doc-schema.json"
      output_format: "markdown"
      timeout_seconds: 600
    - id: "docs:quality-gates"
      description: "Aplicación de 5 gates de calidad documental"
      input_schema: "gate-validation-schema.json"
      output_format: "json"
      timeout_seconds: 300
    - id: "docs:routing"
      description: "Ruteo inteligente a sub-agentes especializados"
      input_schema: "routing-request-schema.json"
      output_format: "json"
      timeout_seconds: 60
...
Coordinador central que recibe solicitudes de documentación, las descompone en subtareas, rutea a agentes especializados, fusiona salidas y hace cumplir gates de calidad.

**Entradas**: Solicitud de usuario, contexto de proyecto, inventario de documentación existente
**Saídas**: Artefactos documentales finales, reporte de validación, ítems de acción
...
7. VALIDAR contra gates de calidad (5 gates)
...
governance_urls:
  ...
  a2a_rules: "https://raw.githubusercontent.com/Mantis-AgenticDev/agentic-infra-docs/refs/heads/main/01-RULES/11-A2A-COMMUNICATION-RULES.md"
```
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/framework/final-deliverable.md",
  "next_agent_hint": "link-validator",
...
## 🧩 Lógica de Orquestación (CEO)

### Protocolo de Ruteo a Sub-Agentes

```bash
#!/usr/bin/env bash
# doc-governance-ceo-orchestrator.sh

source "${MANTIS_ROOT:-.}/docs/framework/doc-governance-ceo.md" --mode=functions-only

orchestrate_doc_generation() {
  local request="${1:?Solicitud de documentación no especificada}"
  local context="${2:-}"
  
  mantis_log "INFO" "orchestration_started" "request:${request}" "unknown" "pt-BR"
  
  # 1. PARSEAR intención
  local doc_type
  doc_type=$(classify_doc_type "$request")
  
  # 2. INVENTARIAR alcance
  local existing_docs
  existing_docs=$(find docs/ -name "*.md" -type f)
  
  # 3. CLASIFICAR tipo de tarea
  local task_type
  if echo "$request" | grep -qE "(generar|crear|escribir)"; then
    task_type="generate"
  elif echo "$request" | grep -qE "(auditar|revisar|evaluar)"; then
    task_type="audit"
  elif echo "$request" | grep -qE "(actualizar|evolucionar|mantener)"; then
    task_type="evolve"
  else
  ...
  case "$doc_type" in
    "api")
      subtasks+=("api-doc-agent")
      ;;
    "architecture")
      subtasks+=("diagram-agent" "adr-agent")
      ;;
    "deployment")
      subtasks+=("deployment-doc-agent")
      ;;
    "tutorial")
      subtasks+=("user-guide-agent")
      ;;
    "event-catalog")
      subtasks+=("event-catalog-agent")
      ;;
    *)
      subtasks+=("code-doc-generator")
      ;;
  esac
  
  # 5. RUTEAR subtareas a agentes (paralelo)
  local outputs=()
  for agent in "${subtasks[@]}"; do
    mantis_log "INFO" "routing_to_agent" "agent:${agent}" "$doc_type" "pt-BR"
    
    # Invocar agente (simulación - en producción llamaría al script del agente)
    local output
    output="${MANTIS_ROOT:-.}/docs/framework/agents/${agent}.md"
  ...
  # 6. COLECTAR salidas
  mantis_log "INFO" "collecting_outputs" "count:${#outputs[@]}" "$doc_type" "pt-BR"
  
  # 7. VALIDAR contra gates de calidad
  local validation_passed=true
  for output in "${outputs[@]}"; do
    if ! validate_frontmatter "$output"; then
      validation_passed=false
      mantis_log "ERROR" "gate_validation_failed" "file:${output},gate:structure" "$doc_type" "pt-BR"
    fi
    
    if ! scan_doc_secrets "$output"; then
      validation_passed=false
      mantis_log "ERROR" "gate_validation_failed" "file:${output},gate:security" "$doc_type" "pt-BR"
    fi
    
    if ! validate_accessibility "$output"; then
      mantis_log "WARN" "gate_validation_warning" "file:${output},gate:accessibility" "$doc_type" "pt-BR"
    fi
  done
  
  # 8. FUSIONAR en entregable coherente
  local final_output="docs/framework/final-deliverable.md"
  cat "${outputs[@]}" > "$final_output"
  
  # 9. REPORTAR resultados
  if $validation_passed; then
    mantis_log "INFO" "orchestration_completed" "output:${final_output}" "$doc_type" "pt-BR"
    echo "$final_output"
    return 0
  else
    mantis_log "ERROR" "orchestration_failed" "validation_errors_detected" "$doc_type" "pt-BR"
    return 1
  fi
}
...
classify_doc_type() {
  ...
  if echo "$request" | grep -qEi "(api|endpoint|rest|graphql)"; then
    echo "api"
  elif echo "$request" | grep -qEi "(arquitectura|diseño|decisión|adr)"; then
    echo "architecture"
  elif echo "$request" | grep -qEi "(despliegue|deployment|infra|ci/cd)"; then
    echo "deployment"
  elif echo "$request" | grep -qEi "(tutorial|guía|how-to|aprender)"; then
    echo "tutorial"
  elif echo "$request" | grep -qEi "(evento|event-driven|catalog)"; then
    echo "event-catalog"
  else
    echo "general"
  fi
...
test_doc_governance_ceo_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local request="Generar documentación de API REST"
  
  # Act
  local result
  result=$(orchestrate_doc_generation "$request")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "api-doc-agent" "$result" || return 1
  return 0
}

test_doc_governance_ceo_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local request="" # Solicitud vacía
  
  # Act
  local result
  result=$(orchestrate_doc_generation "$request" 2>&1) || true
...
test_doc_governance_ceo_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-doc-no-frontmatter.md"
  echo "# Test Doc" > "$test_file"
  
  # Act
  local result
  result=$(validate_frontmatter "$test_file" 2>&1) || true
  
  # Assert
  echo "$result" | grep -q "DOC-C5" || return 1
  echo "$result" | grep -q "Frontmatter YAML ausente" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/doc-governance-ceo.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
