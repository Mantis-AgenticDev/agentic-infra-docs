---
artifact_id: "agent-instruction-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C1","C2","C8","C9","DOC-C1","DOC-C4","DOC-C5"]
canonical_path: "docs/framework/agents/agent-instruction-agent.md"
tier: 2
agent_role: "governance"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/agent-instruction-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 agent-instruction-agent – Generación de Instrucciones para Agentes Documentales

> **Princípio**: Generación de instrucciones optimizadas para agentes documentales.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "agent-instruction-agent"
  ...
  skills:
    - id: "docs:agent-instruction-generation"
      description: "Generación de instrucciones optimizadas para agentes documentales"
...
Generación de instrucciones optimizadas para agentes documentales.

**Entradas**: Plantillas de agente, especificaciones de tarea, contexto de dominio
**Saídas**: System prompts, guidelines de generación, ejemplos few-shot
**Métodos**: Prompt engineering, optimización de contexto, inyección de constraints, ejemplificación de patrones deseados
...
SPAN_ID=$(uuidgen)
AGENT_NAME="agent-instruction-agent"
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/prompts/new-agent-instruction.md",
...
test_agent_instruction_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local agent_spec="Generate API documentation for payment endpoints"
  
  # Act
  local result
  result=$(generate_agent_instructions "$agent_spec")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "system_prompt" "$result" || return 1
  return 0
}

test_agent_instruction_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  ...
  # Act
  local result
  result=$(generate_agent_instructions "$invalid_spec" 2>&1) || true
...
test_agent_instruction_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-instruction.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(validate_frontmatter "$test_file")
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/agent-instruction-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
