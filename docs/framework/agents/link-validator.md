---
artifact_id: "link-validator"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C5","C7","C8","DOC-C1","DOC-C2","DOC-C5","DOC-C7"]
canonical_path: "docs/framework/agents/link-validator.md"
tier: 2
agent_role: "auditor"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/link-validator.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 link-validator – Validación de Integridad de Enlaces

> **Princípio**: Validación de integridad de enlaces internos y externos.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "link-validator"
  ...
  skills:
    - id: "docs:link-validation"
      description: "Validación de integridad de enlaces internos y externos"
...
Validación de integridad de enlaces internos y externos.

**Entradas**: Conjunto de documentos, lista de enlaces a validar
**Saídas**: Reporte de enlaces rotos (404s, redirects, timeouts), recomendaciones de corrección
**Métodos**: Crawling de enlaces, HEAD requests, detección de anchors rotos, verificación de rutas relativas
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reports/link-validation-report.json",
...
test_link_validator_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local test_file="/tmp/test-links.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(validate_links "$test_file")
...
test_link_validator_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local test_file="/tmp/test-broken-links.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(validate_links "$test_file" "/tmp" 2>&1) || true
  
  # Assert
  echo "$result" | grep -q "Enlace interno roto" || return 1
  rm -f "$test_file"
  return 0
}

test_link_validator_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/link-validator.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
