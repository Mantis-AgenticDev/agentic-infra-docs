---
artifact_id: "coverage-mapper"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C5","C7","C8","DOC-C1","DOC-C2","DOC-C5","DOC-C7"]
canonical_path: "docs/framework/agents/coverage-mapper.md"
tier: 2
agent_role: "auditor"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/coverage-mapper.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 coverage-mapper – Mapeo de Cobertura Documental vs Superficie de API

> **Princípio**: Mapeo de cobertura documental vs superficie de API/código.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "coverage-mapper"
  ...
  skills:
    - id: "docs:coverage-mapping"
      description: "Mapeo de cobertura documental vs superficie de API/código"
...
Mapeo de cobertura documental vs superficie de API/código.

**Entradas**: Código fuente, specs OpenAPI, documentación existente
**Saídas**: Mapa de cobertura, endpoints/funciones sin documentar, reporte de gaps
**Métodos**: Parsing de código para símbolos públicos, comparación con índices documentales, detección de exports no documentados, matching de firmas de función
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reports/coverage-map.json",
...
test_coverage_mapper_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local code_file="/tmp/test-api.py"
  local docs_dir="/tmp/test-docs"
  mkdir -p "$docs_dir"
  cat > "$code_file" <<'EOF'
def public_function():
    """Documented function"""
...
EOF
  ...
  # Act
  local result
  result=$(map_coverage "$code_file" "$docs_dir")
  
  # Assert
  echo "$result" | jq -e '.coverage_percentage' || return 1
  rm -f "$code_file"
  rm -rf "$docs_dir"
  return 0
}

test_coverage_mapper_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local code_file="/tmp/test-undocumented.py"
  local docs_dir="/tmp/empty-docs"
  mkdir -p "$docs_dir"
  cat > "$code_file" <<'EOF'
...
EOF
  ...
  # Assert
  echo "$result" | jq -e '.undocumented_endpoints' || return 1
  rm -f "$code_file"
  rm -rf "$docs_dir"
  return 0
}

test_coverage_mapper_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/coverage-mapper.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
