---
artifact_id: "adr-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C5","C6","C8","DOC-C4","DOC-C5","DOC-C6"]
canonical_path: "docs/framework/agents/adr-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/adr-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 adr-agent – Gestión de Architecture Decision Records

> **Princípio**: Gestionar Architecture Decision Records (ADRs).
...
agent_manifesto:
  ...
  skills:
    - id: "docs:architecture-decisions"
...
**Saídas**: Documentos ADR (Contexto, Decisión, Consecuencias, Estado)
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/adr/001-database-choice.md",
...
test_adr_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local decision="Choose PostgreSQL over MongoDB"
  
  # Act
  local result
  result=$(generate_adr "$decision")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "ADR-" "$result" || return 1
  return 0
}

test_adr_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local invalid_decision=""
  
  # Act
  local result
  result=$(generate_adr "$invalid_decision" 2>&1) || true
...
test_adr_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-adr.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "adr-001"
---
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/adr-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
