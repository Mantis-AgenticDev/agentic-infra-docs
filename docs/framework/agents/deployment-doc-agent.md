---
artifact_id: "deployment-doc-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C1","C3","C7","DOC-C1","DOC-C2","DOC-C3","DOC-C5"]
canonical_path: "docs/framework/agents/deployment-doc-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/deployment-doc-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 deployment-doc-agent – Documentación de Infraestructura, CI/CD y Despliegues

> **Princípio**: Documentar infraestructura, pipelines CI/CD, procedimientos de despliegue, estrategias de rollback.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "deployment-doc-agent"
  ...
  skills:
    - id: "docs:deployment-infrastructure"
      description: "Documentación de infraestructura, pipelines CI/CD, procedimientos de despliegue"
...
Documentar infraestructura, pipelines CI/CD, procedimientos de despliegue, estrategias de rollback.

**Entradas**: Dockerfiles, docker-compose, manifiestos Kubernetes, config CI, Terraform/CDK
**Saídas**: Guías de despliegue, referencias de entorno, procedimientos de rollback, diagramas de infraestructura
...
SPAN_ID=$(uuidgen)
AGENT_NAME="deployment-doc-agent"
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/how-to/deployment.md",
  "next_agent_hint": "security-scanner",
...
test_deployment_doc_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local dockerfile="/tmp/test-Dockerfile"
  echo "FROM node:18" > "$dockerfile"
  
  # Act
  local result
  result=$(generate_deployment_docs "$dockerfile")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "deployment" "$result" || return 1
  rm -f "$dockerfile"
  return 0
}

test_deployment_doc_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local invalid_config="/nonexistent/config.yaml"
  
  # Act
  local result
  result=$(generate_deployment_docs "$invalid_config" 2>&1) || true
...
test_deployment_doc_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-deployment-doc.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(scan_doc_secrets "$test_file" 2>&1) || true
  
  # Assert
  echo "$result" | grep -q "DOC-C3" || return 1
  echo "$result" | grep -q "Secret detectado" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/deployment-doc-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
