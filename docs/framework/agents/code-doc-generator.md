---
artifact_id: "code-doc-generator"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C1","C2","C5","DOC-C1","DOC-C2","DOC-C5"]
canonical_path: "docs/framework/agents/code-doc-generator.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/code-doc-generator.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 code-doc-generator – Extracción de Documentación desde Código Fuente

> **Princípio**: Extraer documentación desde código fuente (funciones, clases, módulos, configs).

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "code-doc-generator"
  ...
  skills:
    - id: "docs:code-extraction"
      description: "Extracción de documentación desde código fuente (funciones, clases, módulos)"
...
Extraer documentación desde código fuente (funciones, clases, módulos, configs).

**Entradas**: Archivos fuente, comentarios de código, anotaciones de tipo
**Saídas**: Referencias de API, mejoras de doc inline, visiones generales de módulos
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reference/code-reference.md",
...
test_code_doc_generator_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local source_file="/tmp/test-source.py"
  echo 'def hello(): """Say hello""" pass' > "$source_file"
  
  # Act
  local result
  result=$(generate_code_docs "$source_file")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "hello" "$result" || return 1
  rm -f "$source_file"
  return 0
}

test_code_doc_generator_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local source_file="/nonexistent/file.py"
  
  # Act
  local result
  result=$(generate_code_docs "$source_file" 2>&1) || true
...
test_code_doc_generator_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-doc-with-secret.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
---
```python
api_key = "sk-abcd1234567890abcd1234567890abcd1234567890abcd12"
```
EOF
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/code-doc-generator.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
