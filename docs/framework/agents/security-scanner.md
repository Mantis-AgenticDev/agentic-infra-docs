---
artifact_id: "security-scanner"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C3","C8","DOC-C1","DOC-C3","DOC-C5","DOC-C7"]
canonical_path: "docs/framework/agents/security-scanner.md"
tier: 2
agent_role: "governance"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/security-scanner.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 security-scanner – Detección de Vulnerabilidades en Documentación

> **Princípio**: Detección de vulnerabilidades de seguridad en documentación.
...
agent_manifesto:
  ...
  skills:
    - id: "docs:security-scanning"
      description: "Detección de vulnerabilidades de seguridad en documentación"
...
Detección de vulnerabilidades de seguridad en documentación.

**Entradas**: Documentación de código, guías de despliegue, ejemplos de configuración
**Saídas**: Reporte de vulnerabilidades detectadas, severity scores, recomendaciones de remediación
**Métodos**: Scanning de secrets hardcodeados, detección de patrones inseguros, análisis de permisos expuestos, verificación de dependencias vulnerables en ejemplos
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reports/security-scan-report.json",
...
test_security_scanner_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local test_file="/tmp/test-secure.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(scan_security "$test_file")
  
  # Assert
  echo "$result" | jq -e '.vulnerabilities | length == 0' || return 1
  rm -f "$test_file"
  return 0
}

test_security_scanner_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local test_file="/tmp/test-vuln.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
test_security_scanner_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-validate.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/security-scanner.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
