---
artifact_id: "user-guide-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C4","C8","DOC-C2","DOC-C4","DOC-C5","DOC-C8"]
canonical_path: "docs/framework/agents/user-guide-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/user-guide-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 user-guide-agent – Documentación para Usuarios Finales, Tutoriales y Guías

> **Princípio**: Crear documentación para usuarios finales, tutoriales y guías how-to.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "user-guide-agent"
  ...
  skills:
    - id: "docs:user-guides"
      description: "Creación de documentación para usuarios finales, tutoriales y guías how-to"
...
Crear documentación para usuarios finales, tutoriales y guías how-to.

**Entradas**: Especificaciones de features, screenshots de UI, user stories, guías existentes
**Saídas**: Tutoriales paso a paso, guías de features, FAQ, docs de troubleshooting
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/tutorials/getting-started.md",
...
test_user_guide_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local feature_spec="User registration with email verification"
  
  # Act
  local result
  result=$(generate_user_guide "$feature_spec")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "tutorial" "$result" || return 1
  return 0
}

test_user_guide_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  ...
  # Act
  local result
  result=$(generate_user_guide "$empty_spec" 2>&1) || true
...
test_user_guide_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-guide.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  echo "$result" | grep -q "texto no descriptivo" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/user-guide-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
