---
artifact_id: "audience-analyzer"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C5","C7","C8","DOC-C1","DOC-C2","DOC-C5","DOC-C7"]
canonical_path: "docs/framework/agents/audience-analyzer.md"
tier: 2
agent_role: "auditor"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/audience-analyzer.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 audience-analyzer – Análisis de Match entre Audiencia y Contenido

> **Princípio**: Análisis de match entre audiencia objetivo y nivel de contenido.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "audience-analyzer"
  ...
  skills:
    - id: "docs:audience-analysis"
      description: "Análisis de match entre audiencia objetivo y nivel de contenido"
...
Análisis de match entre audiencia objetivo y nivel de contenido.

**Entradas**: Documentación, metadatos de audiencia objetivo, personas definidas
**Saídas**: Scoring de match audiencia-contenido, gaps de prerrequisitos, recomendaciones de ajuste
**Métodos**: Análisis de complejidad de lenguaje, detección de jerga técnica, scoring de nivel de profundidad, identificación de asunciones implícitas
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reports/audience-analysis-report.json",
...
test_audience_analyzer_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local test_file="/tmp/test-audience.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(analyze_audience "$test_file")
  
  # Assert
  echo "$result" | jq -e '.match_score' || return 1
  rm -f "$test_file"
  return 0
}

test_audience_analyzer_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local test_file="/tmp/test-mismatch.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Assert
  echo "$result" | jq -e '.gaps' || return 1
  rm -f "$test_file"
  return 0
}

test_audience_analyzer_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/audience-analyzer.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
