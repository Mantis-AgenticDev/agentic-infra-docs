---
artifact_id: "diagram-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C5","C8","DOC-C2","DOC-C5","DOC-C8"]
canonical_path: "docs/framework/agents/diagram-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/diagram-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 diagram-agent – Representaciones Visuales en Mermaid

> **Princípio**: Crear representaciones visuales de sistemas, flujos y modelos de datos.
...
agent_manifesto:
  ...
  skills:
    - id: "docs:visual-diagrams"
      description: "Creación de representaciones visuales de sistemas, flujos y modelos de datos"
...
Crear representaciones visuales de sistemas, flujos y modelos de datos.

**Entradas**: Estructura de código, descripciones de arquitectura, flujos de secuencia
**Saídas**: Diagramas Mermaid (flowchart, sequence, class, state, ER, C4, gantt, mindmap, gitgraph)
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/explanation/architecture-diagrams.md",
...
test_diagram_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local description="Flujo de autenticación de usuario"
  
  # Act
  local result
  result=$(generate_diagram "$description" "flowchart")
  
  # Assert
  [[ -n "$result" ]] || return 1
  echo "$result" | grep -q "flowchart" || return 1
  return 0
}

test_diagram_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local invalid_type="invalid-diagram-type"
  
  # Act
  local result
  result=$(generate_diagram "test" "$invalid_type" 2>&1) || true
...
test_diagram_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-diagram-doc.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  echo "$result" | grep -q "alt text" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/diagram-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
