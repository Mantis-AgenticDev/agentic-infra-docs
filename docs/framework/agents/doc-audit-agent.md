---
artifact_id: "doc-audit-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C5","C7","C8","DOC-C1","DOC-C2","DOC-C5","DOC-C7"]
canonical_path: "docs/framework/agents/doc-audit-agent.md"
tier: 2
agent_role: "auditor"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/doc-audit-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 doc-audit-agent – Evaluación Integral de Calidad Documental

> **Princípio**: Evaluación integral de calidad documental.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "doc-audit-agent"
  ...
  skills:
    - id: "docs:quality-audit"
      description: "Evaluación integral de calidad documental"
...
Evaluación integral de calidad documental.

**Entradas**: Repositorio de documentación, base de código de proyecto
**Saídas**: Reporte de auditoría con scores por categoría, lista priorizada de remediación
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reports/audit-report.json",
...
test_doc_audit_agent_happy_path() {
  # Arrange
...
EOF
  ...
  # Assert
  echo "$result" | jq -e '.overall_score' || return 1
  rm -rf "$docs_dir"
  return 0
}

test_doc_audit_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local nonexistent_dir="/nonexistent/docs"
  
  # Act
  local result
  result=$(audit_documentation "$nonexistent_dir" 2>&1) || true
...
test_doc_audit_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-bad-structure.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Assert
  echo "$result" | grep -q "Salto de nivel" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/doc-audit-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
