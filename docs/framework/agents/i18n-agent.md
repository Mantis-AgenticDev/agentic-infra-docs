---
artifact_id: "i18n-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C4","C5","DOC-C4","DOC-C5","DOC-V2"]
canonical_path: "docs/framework/agents/i18n-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/i18n-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 i18n-agent – Internacionalización ES/PT de Documentación

> **Princípio**: Gestionar internacionalización y localización de documentación en español y portugués.
...
agent_manifesto:
  ...
  skills:
    - id: "docs:internationalization"
      description: "Gestión de internacionalización y localización de documentación ES/PT"
...
Gestionar internacionalización y localización de documentación en español y portugués.

**Entradas**: Documentación fuente, locales objetivo, memoria de traducción
**Saídas**: Documentación traducida, reportes de paridad
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/tutorials/getting-started.{es,pt}.md",
...
test_i18n_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local source_doc="/tmp/test-source.md"
  cat > "$source_doc" <<'EOF'
---
artifact_id: "test-doc"
version: "1.0.0"
language: "pt-BR"
---
# Tutorial
EOF
  ...
  # Act
  local result
  result=$(generate_i18n_variants "$source_doc" "es,pt")
  
  # Assert
  [[ -f "${source_doc%.md}.es.md" ]] || return 1
  [[ -f "${source_doc%.md}.pt.md" ]] || return 1
  rm -f "$source_doc" "${source_doc%.md}.es.md" "${source_doc%.md}.pt.md"
  return 0
}

test_i18n_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local nonexistent_doc="/nonexistent/file.md"
  
  # Act
  local result
  result=$(generate_i18n_variants "$nonexistent_doc" "es,pt" 2>&1) || true
...
test_i18n_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local base_file="/tmp/test-base.md"
  local es_file="/tmp/test-base.es.md"
...
EOF
  ...
  # Act
  local result
  result=$(validate_i18n_parity "$base_file" 2>&1) || true
  
  # Assert
  echo "$result" | grep -q "DOC-V2" || return 1
  echo "$result" | grep -q "divergente" || return 1
  rm -f "$base_file" "$es_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/i18n-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
