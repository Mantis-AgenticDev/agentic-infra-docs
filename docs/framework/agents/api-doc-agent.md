---
artifact_id: "api-doc-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C1","C3","C5","DOC-C1","DOC-C2","DOC-C3","DOC-C5"]
canonical_path: "docs/framework/agents/api-doc-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/api-doc-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 api-doc-agent – Documentación de API (REST/GraphQL/gRPC)

> **Princípio**: Generar y mantener documentación de API (REST, GraphQL, gRPC, WebSocket).
...
agent_manifesto:
  ...
  skills:
    - id: "docs:api-documentation"
      description: "Generación y mantenimiento de documentación de API (REST/GraphQL/gRPC)"
...
Generar y mantener documentación de API (REST, GraphQL, gRPC, WebSocket).

**Entradas**: Specs OpenAPI, definiciones de rutas, archivos de schema, anotaciones de código
**Saídas**: Referencias de endpoints, ejemplos de request/response, guías de autenticación, catálogos de errores
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/reference/api/endpoints.md",
...
test_api_doc_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local openapi_spec="/tmp/test-openapi.yaml"
  cat > "$openapi_spec" <<'EOF'
openapi: 3.0.0
info:
  title: Test API
  version: 1.0.0
...
EOF
  ...
  # Act
  local result
  result=$(generate_api_docs "$openapi_spec")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "List users" "$result" || return 1
  rm -f "$openapi_spec"
  return 0
}

test_api_doc_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-002"
  local invalid_spec="/tmp/invalid.yaml"
  echo "invalid: yaml: content" > "$invalid_spec"
  
  # Act
  local result
  result=$(generate_api_docs "$invalid_spec" 2>&1) || true
...
test_api_doc_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-api-doc-with-token.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
---
```bash
curl -H "Authorization: Bearer ghp_abcd1234567890abcd1234567890abcd12"
```
EOF
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/api-doc-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
