---
artifact_id: "code-explainer-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C2","C4","C8","DOC-C2","DOC-C4","DOC-C5","DOC-C8"]
canonical_path: "docs/framework/agents/code-explainer-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/code-explainer-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 code-explainer-agent – Transformar Código Complejo en Contenido Educativo

> **Princípio**: Transformar código complejo en contenido educativo comprensible.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "code-explainer-agent"
  ...
  skills:
    - id: "docs:code-explanation"
      description: "Transformación de código complejo en contenido educativo comprensible"
...
Transformar código complejo en contenido educativo comprensible.

**Entradas**: Código fuente, métricas de complejidad, nivel de audiencia
**Saídas**: Walkthroughs paso a paso, explicaciones visuales, rutas de aprendizaje, ejemplos interactivos
...
SPAN_ID=$(uuidgen)
AGENT_NAME="code-explainer-agent"
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "docs/explanation/understanding-algorithm-x.md",
  "next_agent_hint": "accessibility-checker",
...
test_code_explainer_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local complex_code="def fibonacci(n): return n if n <= 1 else fibonacci(n-1) + fibonacci(n-2)"
  
  # Act
  local result
  result=$(explain_code "$complex_code" "beginner")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "fibonacci" "$result" || return 1
  return 0
}

test_code_explainer_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  ...
  # Act
  local result
  result=$(explain_code "$invalid_code" "beginner" 2>&1) || true
...
test_code_explainer_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-explanation.md"
  cat > "$test_file" <<'EOF'
---
artifact_id: "test"
version: "1.0.0"
...
EOF
  ...
  # Act
  local result
  result=$(validate_accessibility "$test_file" 2>&1) || true
  
  # Assert
  echo "$result" | grep -q "DOC-C8" || return 1
  rm -f "$test_file"
  return 0
}
```
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/code-explainer-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
