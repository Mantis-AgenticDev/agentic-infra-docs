---
artifact_id: "event-catalog-agent"
artifact_type: "agentic-skill-definition"
version: "1.0.0"
constraints_mapped: ["C4","C5","DOC-C2","DOC-C4","DOC-C5","DOC-V1","DOC-V2","DOC-V3"]
canonical_path: "docs/framework/agents/event-catalog-agent.md"
tier: 2
agent_role: "generator"
validation_command: "bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh --domain docs --file docs/framework/agents/event-catalog-agent.md --strict"
language: pt-BR
domain: "docs"
subdomain: "framework"
related_files:
...
# 🧠 event-catalog-agent – Documentación EventCatalog para Arquitecturas Event-Driven

> **Princípio**: Generar documentación EventCatalog para arquitecturas event-driven.

## 📋 AGENT MANIFESTO (Normativa de Ingreso al Ecosistema goals/)

```yaml
agent_manifesto:
  agent_id: "event-catalog-agent"
  ...
  skills:
    - id: "docs:event-catalog"
      description: "Generación de documentación EventCatalog para arquitecturas event-driven"
...
Generar documentación EventCatalog para arquitecturas event-driven.

**Entradas**: Definiciones de servicio, schemas de eventos, configuraciones de canales, modelos de dominio
**Saídas**: Archivos MDX de EventCatalog (servicios, eventos, commands, queries, dominios, flujos, canales, lenguaje ubicuo)
...
SPAN_ID=$(uuidgen)
AGENT_NAME="event-catalog-agent"
...
cat > "./goals/${TASK_ID}/artifacts/${AGENT_NAME}/status.json" <<EOF
{
  ...
  "status": "completed",
  "output_ref": "domains/{DomainName}/index.mdx",
...
test_event_catalog_agent_happy_path() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-001"
  local service_def="OrderService with OrderCreated event"
  
  # Act
  local result
  result=$(generate_event_catalog "$service_def")
  
  # Assert
  [[ -f "$result" ]] || return 1
  grep -q "OrderCreated" "$result" || return 1
  return 0
}

test_event_catalog_agent_error_handling() {
  # Arrange
  export TENANT_ID="test-tenant"
  ...
  # Act
  local result
  result=$(generate_event_catalog "$invalid_def" 2>&1) || true
...
test_event_catalog_agent_constraint_validation() {
  # Arrange
  export TENANT_ID="test-tenant"
  export TASK_ID="test-task-003"
  local test_file="/tmp/test-event-catalog.mdx"
  cat > "$test_file" <<'EOF'
---
id: "OrderService"
version: "1.0.0"
...
bash 05-CONFIGURATIONS/validation/orchestrator-engine.sh \
  --domain docs \
  --file docs/framework/agents/event-catalog-agent.md \
  --json \
  --check-diataxis \
  --check-links \
  --check-secrets \
